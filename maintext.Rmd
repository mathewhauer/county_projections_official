...
Thus to find the population of ten to fourteen year olds ($_5P_{10}$) in five years ($t+1$), we add the difference of the population aged 10-14 in time $t$ ($_5P_{10,t}$) to the population aged 5-9 five-years prior in time $t-1$ ($_5P_{5,t-1}$) to the population aged 5-9 in time $t$ ($_5P_{5,t}$). ie, if we have 100 5-9 year olds five years ago and we now have 125 10-14 year olds and 90 5-9 year olds, we can expect the number of 10-14 year olds in 5 years to be (125-100 $+$ 90 = 115). \autoref{lexisplot} demonstrates the similarities of using CCRs and CCDs in a lexis diagram.
...
Where $_{35}W_{15}$ is the population of women in child-bearing ages 15-50. I use the state/race-specific CWRs for member counties.
...
{_{\infty}\hat{P}_{85+t}} & = & {_{\infty}CCR_{85,t}} \,\, \cdot \,\, {_{\infty}P_{85-y,t}}\nonumber\\
{_{\infty}CCD_{85,t}} & = & {_{\infty}P_{85,t}} - {_{\infty}P_{85-y,t-1}}\\
{_{\infty}\hat{P}_{85+t}} & = & {_{\infty}CCD_{85,t}} \,\, + \,\, {_{\infty}P_{85-y,t}}\nonumber
\end{eqnarray}
...
The SSPs are only introduced to control the projections for 2020-2100. The technical validation does not use the SSPs as controls.
...
a <- tribble(
  ...
  '@wilson2016evaluation',	"New South Wales",	"Ten cohort-component and hamilton-perry variants","	Total population",	"Median APE",	"10-years",	"3.6% - 6.5%",
  "@rayer2008population",	"US counties",	"Seven extrapolation approaches",	"Total population",	"Mean APE",	"10-years",	"9.3% - 13.7%",
    "@smith2003evaluation",	'US counties',	"Cohort-component",	"Age Structure",	"Mean APE",	"10-years",	"6.7% - 10.6%",
    "@smith2003evaluation",	"Florida counties",	"Hamilton-Perry/Cohort-component",	"Age Structure",	"Mean APE",	"10-years",	"4.9% - 15.4%",
    "@sprague2012automatic",	"US Counties",	"Optimized Hamilton-Perry", "Age structure",	"Mean APE",	"10-years",	"6% - 16%"
    
)

kable(a, caption="**Comparable Population Projection Errors.**")
```

Overall, the errors reported here are on par with or better than many cohort-component population projection models. 


For example, Smith and Tayman's [@smith2003evaluation] county-level cohort-component model produced age-specific 10-year projection mean APEs between 6.7% (project 1980 to 1990) and 10.6% (project 1990 to 2000). Rayer's [@rayer2008population] various extrapolation approaches produced 10-year total population projection mean APEs averaging 9.6%. Wilson [@wilson2016evaluation] tested various cohort-component and Hamilton-Perry variants in New South Wales finding total population 10-year median APE's between 3.6% and 6.5% -- comparable to my 10-year median APE's between 7.6% and 8.0% (\autoref{tab:agestotal}). Wilson [@wilson2016evaluation] tested various cohort-component and Hamilton-Perry variants in New South Wales finding total population 10-year median APE's between 3.6% and 6.5% -- comparable to my 10-year median APE's between 5.1% and 5.5% (\autoref{tab:COUNTYeval}).  @rayer2008population; @wilson2005recent; @booth2006demographic; @wilson2012forecast;@raftery2012bayesian; @boyle2010projection; @daponte1997bayesian; @lutz1996probabilistic]. 

While overall the ex-post-facto evaluation showed relatively low errors, some areas in the United States, some demographic sub-groups, and some age-groups could exhibit greater error rates. These groups include but are not limited to non-white populations, young child under the age of 5, older adults over the age of 85, and parts of Colorado, New Mexico, and North Dakota.
